drop VIEW `work_shukko_vws`;
CREATE VIEW `work_shukko_vws` AS select 
`a`.`shouhin_mr_cd` AS `shouhin_mr_cd`,
`b`.`moto_tantou_mr_cd` AS `tantou_mr_cd`,
...
`b`.`moto_souko_mr_cd` AS `souko_mr_cd`,
NULL AS `nyuukobis`,
...
`a`.`bikou` AS `bikou`,
(0 - `a`.`suuryou1`) AS `zaiko_ryou1s`,
(0 - `a`.`suuryou2`) AS `zaiko_ryou2s`,
...
(`c`.`uriage_flg` * `a`.`suuryou1`) AS `uriage_ryou1s`,
(`c`.`uriage_flg` * `a`.`suuryou2`) AS `uriage_ryou2s`,
(`c`.`shukko_flg` * `a`.`suuryou1`) AS `hokashukko_ryou1s`,
(`c`.`shukko_flg` * `a`.`suuryou2`) AS `hokashikko_ryou2s`,
...
where ((`a`.`henkansaki_flg` <> 1) and ((`c`.`uriage_flg` <> 0) or (`c`.`shukko_flg` <> 0)))
