$('#fieldCd').change(function() { //条件仕入月報を索引
	$("#name").val($("#fieldCd option:selected").text());
	$.ajax({
		type:"POST",
		url:jouken_shiire_geppous_ajaxGet,
		data:{'cd':$(this).val(),},
		async:true,
		dataType:'json',
...
$("#fieldKikanFrom").change(function() { //期間からが変更されたら
	$("[name='kikan_sitei_kbn_cd']").val('1519'); // 1519:任意の期間
});
...
function init_del() {
	...
	if (ret) {
		location.href=jouken_shiire_geppous+"/modaldel/"+$("#fieldCd").val();
	}
}
