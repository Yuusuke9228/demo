$('#fieldCd').change(function() { //条件売掛残高を索引
	$.ajax({
		type:"POST",
		url:jouken_urikake_zandakas_ajaxGet,
		data:{'cd':$(this).val(),},
		async:true,
		dataType:'json',
...
function f8key() {
	if (lastfocusin == "fieldCd") { /* 条件名選択 */
		modalstart(jouken_urikake_zandakas_modal); //条件設定画面を表示
...
$('#dl-xlsx').on('click', function () {
	...
	saveAs(blob, 'urikake_zandakas.xlsx');
});
