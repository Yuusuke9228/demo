...
$(function () {
    $('.tanka')
        ...
        .focusout(function (e) {
            ...
            var shiiresaki_mr_cd = $('#shiiresaki_mr_cd').val();
            ...
            // 仕入先未選択時は更新しない
            if (!shiiresaki_mr_cd) {
                window.alert("仕入先が未選択です。");
                return false;
            }

            row_data = {
                'shouhin_mr_cd': shouhin_mr_cd,
                'shiiresaki_mr_cd': shiiresaki_mr_cd,
                'tanka': tanka
            };
            $.ajax({
                url: shiiresaki_tanka_save_ajax,
                dataType: "json",
                type: "POST",
                data: row_data,
                success: function (res) {
                    ...
                    //console.log(res);
                    //console.log("---------------------------------");
                    window.alert(shouhin_mr_cd + " の " + shiiresaki_mr_cd + " 用、単価を設定しました!!");
                },
...
/** ---------------------------------- Modal ---------------------------------- */
function f8key() {
    if (lastfocusin === "shiiresaki_mr_cd") {
 ...
 * 仕入先名称取得
 */
$('#shiiresaki_mr_cd').change(function () {
    $.ajax({
        type: "POST",
        ...
        success: function (data) {
            if (data.length === 0) {
                $("#shiiresaki_name").val('Error: 未登録');
            } else if (data.length == 1 || $("#shiiresaki_mr_cd").val() === data[0].cd) {
                $("#shiiresaki_name").val(data[0].name);
            }
        },
        error: function (xhr, status, err) {
            $("#shiiresaki_name").val('Error: Api Error ShiiresakiMrsController@AjaxGetAction');
        },
    });
});
...
$('#dl-xlsx').on('click', function () {
    ...
    saveAs(blob, 'ShiiresakiBetsu_Tankalist.xlsx');
    return false;
});
