$('#fieldCd').change(function() { //条件仕入明細を索引
	$("#name").val($("#fieldCd option:selected").text());
	$.ajax({
		type:"POST",
		url:jouken_shiire_meisais_ajaxGet,
		data:{'cd':$(this).val(),},
		async:true,
		dataType:'json',
...
$('#fieldUtiwakeKbns').change(function() { // 内訳区分を変更したら
	$('#fieldUtiwakeKbns option').each(function(){
//alert('#'+$(this).val()+" "+$(this).prop('selected'));
...
function init_del() {
	...
	if (ret) {
		location.href=jouken_shiire_meisais+"/modaldel/"+$("#fieldCd").val();
	}
}
