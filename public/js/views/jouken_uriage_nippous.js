$('#fieldCd').change(function() { //条件売上日報を索引
	$("#name").val($("#fieldCd option:selected").text());
	$.ajax({
		type:"POST",
		url:jouken_uriage_nippous_ajaxGet,
		data:{'cd':$(this).val(),},
		async:true,
		dataType:'json',
		success: function (data) {
			if(data.length==0){
				...
				$("[name='torihiki_kbn_betu_flg']").val(flg=data[0].torihiki_kbn_betu_flg).prop("checked",(flg==1));
				$("[name='junjo_kbn_cd']").val(data[0].junjo_kbn_cd);
				$("[name='junjo_kbn_table']").val(data[0].junjo_kbn_table);
				$("[name='torihiki_kbn_cd']").val(data[0].torihiki_kbn_cd);
				...
				$("[name='simekiri_kbn']").val(data[0].simekiri_kbn);
...
function f8key() {
	if (lastfocusin == "fieldHanniFrom" || lastfocusin == "fieldHanniTo") { /*  範囲からまで  */
		hanni_modal = base_uri+$("#junjo_kbn_table").val()+"_modal";
...
$("#fieldKikanFrom").change(function() { //期間からが変更されたら
	$("[name='kikan_sitei_kbn_cd']").val('1219'); // 1219:任意の期間
});
...
function init_del() {
	...
	if (ret) {
		location.href=jouken_uriage_nippous+"/modaldel/"+$("#fieldCd").val();
	}
}
