$('#fieldCd').change(function() { //条件仕入順位を索引
	$.ajax({
		type:"POST",
		url:jouken_shiire_ajaxGet,
		data:{'cd':$(this).val(),},
		async:true,
		dataType:'json',
...
function f8key() {
	if (lastfocusin === "fieldCd") { /* 条件名選択 */
		modalstart(jouken_shiire_modal); //条件設定画面を表示
...
$("#fieldKikanFrom").change(function() { //期間からが変更されたら
	$("[name='kikan_sitei_kbn_cd']").val($("[name='kikan_sitei_kbn_cd']").val().substr(0,2)+'19');
});
