...
$(".zoom_hacchuu").click(function ()
{
    ...
    $("<input>", {
        type: 'hidden',
        name: 'data[hacchuu_meisai_dts][0][shouhin_mr_cd]',
        id: 'hacchuuMeisaiDts0ShouhinMrCd',
        value: $(this).prevAll().eq(6).text()
    }).appendTo('#post_hacchuu');
    $("<input>", {
        type: 'hidden',
        name: 'data[hacchuu_meisai_dts][0][suuryou]',
        id: 'hacchuuMeisaiDts0Suuryou',
        value: $(this).text().replace(/,/g, '')
    }).appendTo('#post_hacchuu');
    $("<input>", {
        type: 'hidden',
        name: 'data[hacchuu_meisai_dts][0][tanni_mr_cd]',
        id: 'hacchuuMeisaiDts0TanniMrCd',
        value: $("td", $(this).parent().nextAll(0)).eq($(this).index() - 2).text()
    }).appendTo('#post_hacchuu');
    $("#post_hacchuu").submit();
});
...
$(".zoom_hacchuu_all").click(function ()
{
    ...
    for (j = 0; j < i; j++)
    {
        $("<input>", {
            type: 'hidden',
            name: 'data[hacchuu_meisai_dts][' + j + '][shouhin_mr_cd]',
            id: 'hacchuuMeisaiDts' + j + 'ShouhinMrCd',
            value: $("td", $(this).parent().prevAll().eq(2 * (i - j) - 1)).eq($(this).index() - 7).text()
        }).appendTo('#post_hacchuu');
        $("<input>", {
            type: 'hidden',
            name: 'data[hacchuu_meisai_dts][' + j + '][suuryou]',
            id: 'hacchuuMeisaiDts' + j + 'Suuryou',
            value: $("td", $(this).parent().prevAll().eq(2 * (i - j) - 1)).eq($(this).index()).text().replace(/,/g, '')
        }).appendTo('#post_hacchuu');
        $("<input>", {
            type: 'hidden',
            name: 'data[hacchuu_meisai_dts][' + j + '][tanni_mr_cd]',
            id: 'hacchuuMeisaiDts' + j + 'TanniMrCd',
            value: $("td", $(this).parent().prevAll().eq(2 * (i - j) - 2)).eq($(this).index() - 2).text()
        }).appendTo('#post_hacchuu');
    }
    $("#post_hacchuu").submit();
});
...
function f8key()
{
    ...
    } else if (lastfocusin == "fieldKikanTuki")
    { /* 期間月選択 */
        open_datepicker();
    }
}
